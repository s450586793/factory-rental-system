...
- `2026-09-25 09:47 CST` `V0.9.4` 修复中文附件文件名乱码，新上传正确保存名称，升级时自动修复已有可恢复的乱码名称，保留附件内容、路径和合同关联；合同历史列表移除“预收”列，操作区简化为“上传 / 合同 / 编辑 / 删除”四个按钮。
- `2026-09-25 09:17 CST` `V0.9.3` 已签合同单文件上传上限提升至 100 MB，同步调整后端校验、错误提示和 Nginx 上传请求限制；多份已签合同逐个上传，中途失败后重试复用已成功上传的附件。
- `2026-09-21 09:48 CST` `V0.9.2` 合同列表移除“金额历史”按钮及对应弹窗，后台继续保留金额修改记录和查询接口，便于核对租金、押金与水电单价的变更。
- `2026-09-20 12:51 CST` `V0.9.1` 房租收费恢复为收款记录列表，移除应收计划标签、期次查看和收款分配预览。新合同自动按租赁周年在后台生成年度应收，只有已到付款日的年度计入应收和欠费，完整多年合同仍只保存一份；已有半年收费记录及收款分配继续保留。合同列表增加“上传已签合同”入口，支持 PDF、图片选择或拖拽、重复提交保护和附件预览；补传只追加附件，不修改合同金额或重新生成 PDF。
...
- 水表/电表配置与水电收费记录
- 房租收费记录、押金记录
- 多年合同后台按租赁周年计费，未来年度不计入当前欠费
- 合同列表直接补传已签合同 PDF / 图片，保留历史附件与预览
- 收据 PDF 生成、查看与作废
