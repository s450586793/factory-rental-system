...
- `contract_document_jobs`
  每份合同对应一条 PDF 生成任务，保存内容版本、生成载荷、状态、尝试次数及失败原因。任务在独立 Node 子进程中渲染，单次超时 60 秒；当前单后端部署会自动恢复遗留的处理中任务。生成文件保存于持久化存储，按内容版本复用。

- `contract_financial_history`
  合同金额修改历史，与合同同事务写入，记录操作者、时间、字段、新旧值。记录年租金、押金、提前退租违约金、电费单价、电费线损和水费单价；首次保存记录初值，值未改变不增加记录。历史合同从升级后的首次修改开始记录，不推测升级前的历史。
